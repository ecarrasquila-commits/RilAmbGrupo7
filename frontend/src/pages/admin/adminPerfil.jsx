...
.rl-sidebar::before {
  ...
  background:linear-gradient(90deg,transparent,rgba(255,255,255,.90) 50%,transparent);
  pointer-events:none;
}
.rl-sidebar.open { transform:translateX(0)!important; }

.rl-sb-brand { padding:28px 22px 24px; display:flex; align-items:center; gap:12px; }
.rl-sb-close-btn {
  display:none; margin-left:auto; width:28px; height:28px; border-radius:8px;
  border:0.5px solid var(--i10); background:var(--i04);
  align-items:center; justify-content:center; cursor:pointer; color:var(--i40); flex-shrink:0;
}
.rl-sb-mark {
  width:34px; height:34px; background:var(--ink); border-radius:10px;
  display:flex; align-items:center; justify-content:center; flex-shrink:0;
  box-shadow:0 2px 8px rgba(0,8,44,.22),inset 0 1px 0 rgba(255,255,255,.10);
}
.rl-sb-mark svg { color:rgba(255,255,255,.85); }
.rl-sb-name { font-size:16px; font-weight:600; color:var(--ink); letter-spacing:-.02em; }
.rl-sb-tag  { font-size:9.5px; font-weight:500; color:var(--i40); letter-spacing:.09em; text-transform:uppercase; margin-top:2px; }
.rl-sb-nav  { padding:20px 14px; flex:1; overflow-y:auto; }
.rl-sb-item {
  display:flex; align-items:center; gap:11px; height:40px; padding:0 12px;
  border-radius:var(--r-sm); border:0.5px solid transparent;
  cursor:pointer; transition:all .17s ease; margin-bottom:2px;
  text-decoration:none; color:inherit; position:relative; background:none;
  width:100%;
}
.rl-sb-item svg   { color:var(--i40); flex-shrink:0; transition:color .17s; }
.rl-sb-item span  { font-size:13px; font-weight:400; color:var(--i50); transition:color .17s; letter-spacing:.005em; }
.rl-sb-item:hover { background:var(--i04); border-color:var(--i07); }
.rl-sb-item:hover svg, .rl-sb-item:hover span { color:var(--i70); }

.rl-sb-footer { padding:16px 14px; border-top:0.5px solid var(--i07); position:relative; }
.rl-sb-profile {
  display:flex; align-items:center; gap:11px; padding:10px 12px;
  border-radius:var(--r-sm); border:0.5px solid transparent; cursor:pointer; transition:all .17s;
}
.rl-sb-profile:hover { background:var(--i04); border-color:var(--i07); }
.rl-sb-av { width:34px; height:34px; border-radius:9px; background:var(--ink); border:0.5px solid var(--i10); display:flex; align-items:center; justify-content:center; font-size:12px; font-weight:600; color:rgba(255,255,255,.85); flex-shrink:0; }
.rl-sb-pname { font-size:12.5px; font-weight:500; color:var(--i70); }
.rl-sb-prole { font-size:10.5px; color:var(--i40); margin-top:1px; }
.rl-sb-dots  { margin-left:auto; color:var(--i24); }

/* Dropdown */
.rl-sb-dropdown {
  display:none; position:absolute; bottom:100%; left:14px; right:14px; margin-bottom:8px;
  background:var(--card); border:0.5px solid var(--i10); border-radius:var(--r-sm);
  box-shadow:0 4px 16px rgba(0,8,44,.12); overflow:hidden; z-index:40; animation:fadeUp .2s ease both;
}
.rl-sb-dropdown.open { display:block; }
.rl-sb-dropdown-item {
  display:flex; align-items:center; gap:10px; padding:10px 12px;
  cursor:pointer; transition:background .17s; text-decoration:none; color:inherit; background:none; border:none; width:100%;
}
...
.rl-sb-divider { height:0.5px; background:var(--i07); margin:0; }
...
function PasswordSection() {
  ...
  // Expose showToast via ref from parent
  ...
  const handleSendCode = () => {
    setLoading(true);
    setTimeout(() => { setLoading(false); goToStep(2); }, 1600);
  };
  ...
  const handleVerify = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (code === '000000') {
        setAlert({ visible: true, type: 'danger', msg: 'Código incorrecto. Inténtalo de nuevo.' });
  ...
  const handleSavePwd = () => {
    ...
    setTimeout(() => {
      ...
      goToStep(1);
      // trigger toast via window event
      ...
      {step === 1 && (
        <div className="rl-pwd-step">
              ...
              <div className="rl-pwd-field">
                ...
                <div className="rl-pwd-input-wrap">
                  <input type="email" className="rl-pwd-input"
                    value="admin@rilambplatform.co" readOnly
          ...
          <div className="rl-pcard-foot">
            <button className="rl-btn-save" onClick={handleSendCode} disabled={loading}>
      ...
      {step === 3 && (
        <div className="rl-pwd-step">
          ...
          <div className="rl-pcard-foot">
            ...
            <button className="rl-btn-save" onClick={handleSavePwd} disabled={loading}>
              {loading ? <span className="rl-btn-spinner" /> : <IconCheck />}
...
function GeneralSection({ showToast }) {
  ...
  const [apellido, setApellido] = useState('Sánchez');
  const [cedula, setCedula]   = useState('10000000001');
  const [telefono, setTelefono] = useState('+57 300 000 0000');
  const [correo, setCorreo]   = useState('admin@rilambplatform.co');
  ...
  return (
    <div className="rl-pcard">
        ...
        <div className="rl-pf-grid">
          ...
          <div className="rl-pf-group">
            ...
            <input className={`rl-pf-input${cedulaErr ? ' error' : ''}`} type="text"
              ...
              onChange={e => { setCedula(e.target.value); validateCedula(e.target.value); }} />
            {cedulaErr && <span style={{ fontSize: 11, color: 'var(--red)', marginTop: 2 }}>Ingresa un número de cédula válido</span>}
          </div>
          <div className="rl-pf-group">
      ...
      <div className="rl-pcard-foot">
        ...
        <button className="rl-btn-save" onClick={() => showToast('Perfil actualizado correctamente')}>
          <IconCheck /><span>Guardar cambios</span>
        </button>
      </div>
    </div>
  );
    ...
    <div className="rilambRoot" style={{ minHeight: '100vh', background: 'var(--page)', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <aside className={`rl-sidebar${sidebarOpen ? ' open' : ''}`}>
        ...
        <nav className="rl-sb-nav">
          <button className="rl-sb-item" onClick={() => navigate('/admin/reports')}><IconFile /><span>Reportes</span></button>
          <button className="rl-sb-item" onClick={() => navigate('/admin/users')}><IconUsers /><span>Usuarios</span></button>
          <button className="rl-sb-item" onClick={() => navigate('/admin/devices')}><IconDevice /><span>Dispositivos</span></button>
        </nav>
        <div className="rl-sb-footer">
          <div className="rl-sb-profile" onClick={() => setDropdownOpen(p => !p)}>
            <div className="rl-sb-av">AS</div>
            <div><div className="rl-sb-pname">Admin S.</div><div className="rl-sb-prole">Administrador</div></div>
            <span className="rl-sb-dots"><IconDots /></span>
          </div>
          <div className={`rl-sb-dropdown${dropdownOpen ? ' open' : ''}`}>
            <button className="rl-sb-dropdown-item" onClick={() => switchSection('general')}>
      ...
      <div className="rl-layout">
        ...
        <div className="rl-content">
          <header className="rl-topbar">
            ...
            <button className="rl-tb-ico">
              <IconBell />
              <span className="rl-notif-dot" />
            </button>
            <button className="rl-tb-avatar" onClick={handleLogout}><IconLogout /></button>
          </header>
            ...
            <div className="rl-profile-shell">
              ...
              <div className="rl-identity-panel">
                <div className="rl-av-block">
                  ...
                  <div className="rl-av-name">Admin Sánchez</div>
                  <div className="rl-av-role"><span className="rl-av-role-dot" />Administrador</div>
                </div>
